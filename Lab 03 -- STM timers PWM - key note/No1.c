...
void TIM_OC_Config(void) {
    ...
    TIM_OC_InitStruct.CompareValue = TIMx_ARR * 0.01;              //  1% duty
...
void EXTI0_IRQHandler(void) {
    ...
    if(LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_0) == SET) {
        ...
        uint32_t CCRx = LL_TIM_OC_GetCompareCH1(TIM4);
        if (CCRx < TIMx_ARR * 0.2) {
            CCRx = TIMx_ARR * 0.2;
        } else if (CCRx < TIMx_ARR) {
            CCRx += TIMx_ARR * 0.2;
        } else {
            CCRx = TIMx_ARR * 0.01;
        }
        LL_TIM_OC_SetCompareCH1(TIM4, CCRx);
    }
}
